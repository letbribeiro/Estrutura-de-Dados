...
int qntd_nos(TNoA* a){
    ...
    return 1+ qntd_nos(a->dir) + qntd_nos(a->esq);
}

void percorrer(TNoA* a, char vet[], int* i){
    if (a!= NULL){
        vet[*i] = a->info;
        (*i)++;
        percorrer(a->esq, vet, i);
        percorrer(a->dir, vet, i);
    }
}
/*
...
int mesmos_nos(TNoA *a1, TNoA *a2) {
    ...
    int idx1 = 0;
    int idx2 = 0;
    percorrer(a1, vet1, &idx1);
    percorrer(a2, vet2, &idx2);
    int achou = 0;
