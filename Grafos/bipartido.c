...
int ehbipartido(TGrafo *g) {
    ...
    while (aux != NULL){
        if (aux->cor == -1){
            aux->cor = 0;
            push(p, aux->id_vertice);
        }
        while(pilha_vazia(p) != 1){
            int id_i = pop(p);
            TGrafo* i = busca_vertice(g, id_i);
            TVizinho* v = i->prim_vizinho;
            while (v != NULL){
                TGrafo* v_grafo = busca_vertice(g, v->id_vizinho);
                if (v_grafo->cor == -1){
                    if (i->cor == 0) v_grafo->cor = 1;
                    else v_grafo->cor = 0;
                    push(p, v_grafo->id_vertice);
                } else if (v_grafo->cor == i->cor){
                    libera(p);
                    return 0;
                } 
                v = v->prox;
